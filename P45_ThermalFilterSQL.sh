...
read -p 'Please enter a vario integration value:' Integrator
...
DoneFolder=$Syride/$Site/${Site}"_done_i_"${Integrator}
...
DoneFolder=$Syride/$Site/${Site}"_thermal_"${Therm_Time_abs}
table=$(basename $DoneFolder)
...
for files in ${DoneFolder}/*
	...
	COPY ${table} FROM '$files' (DELIMITER(','));
